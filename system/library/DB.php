...
class DB extends Kernel {
	...
	public static function connect($database='default') {
		...
		self::query('SET NAMES \''.$conf['charset'].'\'', false, $database);
	}
	...
	*/
	 ...
	 *	\see execute
	 */
	public static function query($sql, $returnRows=false, $database='default') {
		return self::execute($sql, $returnRows, $database);
	}
	 ...
	 */
	public static function get_all($database='default', $resultType=DB_RESTYPE_ASSOC) {
		if (isset(self::$SQLRes[$database])) {
			switch ($resultType) {
