...
class Kernel {
	...
	public static function debug_print() {
		if (self::get_conf('system', 'debug') == true && !empty(self::$debug)) {
			...
			echo '.debug_box { font-size:11px; font-family:Verdana; color:#000000; background: #FFF; margin:0; width:99%; display:block; position:absolute; top:0; left:0; text-decoration:none; text-align:center; border: 2px solid #F93; border-top:none; z-index:9999; }';
			...
			echo '.debug_box_3 { background-image:url(data:image/gif;base64,); font-weight: bold; color:#F93; }';
			echo '</style>';
			echo '<div class="debug_box">';
			echo '<div class="debug_box_top_asd" onmouseover="this.className=\'debug_box_top_asd_hover\'" onmouseout="this.className=\'debug_box_top_asd\'"><div class="debug_box_2">';

			for ($i=0; $i<count(self::$debug); $i++) {
				echo ($i>0 ? "<br />\n<hr /><br />\n" : '') . self::$debug[$i];
			}

			echo '</div><div class="debug_box_3">D E B U G</div></div></div>';
		}
	}
