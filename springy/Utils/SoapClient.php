...
class SoapClient extends GlobalSoapClient
{
    ...
    public function __construct(?string $wsdl, array $options, array $wsse)
    {
        ...
        $options += (
            config_get('network.soap.proxy.host') ? [
                'proxy_host' => config_get('network.soap.proxy.host'),
                'proxy_port' => config_get('network.soap.proxy.port'),
            ] : []
        ) + (
            config_get('network.soap.proxy.user') ? [
                'proxy_login' => config_get('network.soap.proxy.user'),
                'proxy_password' => config_get('network.soap.proxy.password'),
            ] : []
        ) + (
            ['connection_timeout' => config_get('network.soap.timeout') ?: 20]
        ) + (
            $this->isUsingWsse($wsse) ? ['trace' => true] : []
        );
