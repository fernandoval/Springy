...
class Validator 
{
     ...
     * \return \static
     */
    public static function make($input = array(), array $rules = array(), $messages = array())
    {
     ...
     * \param [in] (array) $messages - Mensagens de erros para cada campo
     */
    public function __construct($input = array(), array $rules = array(), $messages = array())
    {
        $this->setInput($input);
        $this->rules = $rules;
    ...
    {
        if ( $this->errors->hasAny() ) $this->errors = new MessageContainer;
    ...
    {        
        foreach ($this->explodeRules($rules) as $rule)
        {
            ...
            if ( ! call_user_func(array($this, $rule['method']), $field, $rule['params']) ) {
    ...
    protected function parseRule($rule)
    {
        ...
        return array(
            ...
            'params' => isset($ruleAndParams[1]) ? (array)explode(',', $ruleAndParams[1]) : array()
        );
    }
    ...
    protected function parseErrorMessage($field, $rule)
    {
        ...
        return str_replace(array(':field',':rule'), array($field, $rule), $message);
    }
    ...
    {
        if ( ! isset($this->input[$field]) ) return true;
