...
class Kernel {
	...
	public static function debug_print() {
		...
		if (self::get_conf('system', 'debug') == true && !empty(self::$debug) && !in_array('Content-type: application/json', $headers)) {
			echo '<style type="text/css">';
			echo '.debug_box {color:#000000; background: #FFF; margin:0; width:99%; display:block; position:absolute; top:0; left:0; text-decoration:none; text-align:center; border: 2px solid #F93; border-top:none; z-index:9999; }';
			echo '.debug_box * { font-size:11px; font-family:Verdana }';
			echo '.debug_box_top_asd .debug_box_2 { display:none }';
			echo '.debug_box_top_asd_hover .debug_box_2 { border-top:2px solid #F93; display:block; height:300px; color:#000000; overflow:auto; text-align:left; padding:0 5px }';
			echo '.debug_box_3 { cursor:pointer; font-weight: bold; color:#F93; background-image:url(data:image/gif;base64,) }';
			echo '.debug_box_2 table thead th {/*background:#FFC68C;*/ border-top: 1px solid #000; font-weight: bold} ';
			echo '</style>';
			echo '<div class="debug_box">';
			echo '<div class="debug_box_top_asd" id="debug_box_top"><div class="debug_box_2">';
			
			echo implode('', self::$debug);
			
			echo '</div><div class="debug_box_3" onclick="var id = document.getElementById(\'debug_box_top\'); if (id.className==\'debug_box_top_asd\') {id.className=\'debug_box_top_asd_hover\';} else {id.className=\'debug_box_top_asd\';}">D E B U G</div></div></div>';
		}
	}
