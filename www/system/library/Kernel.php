...
class Kernel
{
    ...
    public static function printCopyright()
    {
        ...
        foreach (self::_list_classes($d, 'Springy') as $k => $v) {
            echo '<tr><td>'.str_replace('\\', '<span class="slash">\\</span>', str_replace('Springy\\', '<span class="fw">Springy</span>\\', str_replace('class ', '<span class="class">class</span> ', str_replace('interface ', '<span class="class">interface</span> ', $v['n'])))).'</td><td class="version">'.$v['v'].($v['b'] ? '</td><td class="description">'.$v['b'] : '').'</td></tr>';
        }
        echo '</table>';
